...
const Homepage = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        // Fetch hero section data (if you choose to fetch it)
        // const heroResponse = await fetch(`${import.meta.env.VITE_API_URL}/api/hero`);
        // if (!heroResponse.ok) throw new Error('Hero data fetch failed');
        // const heroData = await heroResponse.json();
        setHeroData(heroData);
    
        // Fetch featured trips
        const tripsResponse = await fetch(`${import.meta.env.VITE_API_URL}/api/featured-trips`);
        if (!tripsResponse.ok) throw new Error('Featured trips fetch failed');
        const tripsData = await tripsResponse.json();
        setFeaturedTrips(tripsData);
    
        // Fetch trekking services
        const servicesResponse = await fetch(`${import.meta.env.VITE_API_URL}/api/featured-service`);
        if (!servicesResponse.ok) throw new Error('Trekking services fetch failed');
    ...
    <div className="bg-gray-50 text-gray-900 font-serif">
        ...
        <section className="relative bg-cover bg-center h-[70vh]" style={{ backgroundImage: `url(${heroData.imageUrl})` }}>
          ...
          <div className="container mx-auto text-center text-white relative z-10 py-32">
            ...
            <Link to={heroData.buttonLink}>
              <button className="mt-8 px-10 py-4 bg-gradient-to-r from-blue-500 to-blue-700 text-white rounded-full shadow-xl hover:scale-105 hover:shadow-2xl transition-all ease-in-out duration-300" aria-label="Book your adventure">
                {heroData.buttonText}
              </button>
            </Link>
          </div>
                ...
                <div key={trip.id} className="border border-gray-300 rounded-lg overflow-hidden shadow-lg hover:shadow-xl transition transform hover:scale-105 duration-300">
                  ...
                  <div className="p-6 bg-white">
                    ...
                    <Link to={trip.bookingLink}>
                      <button className="mt-4 px-8 py-3 bg-gradient-to-r from-blue-500 to-blue-700 text-white rounded-lg hover:scale-105 hover:shadow-xl transition duration-300" aria-label="Book Now">
                        Book Now
                      </button>
                    </Link>
                  </div>
              ...
              {trekkingServices.map((service) => (
                <div key={service.id} className="border border-gray-300 rounded-lg p-6 shadow-lg hover:shadow-xl transition duration-300">
                  ...
                  <Link to={service.detailLink}>
                    <button className="mt-4 px-8 py-3 bg-gradient-to-r from-blue-500 to-blue-700 text-white rounded-lg hover:scale-105 hover:shadow-xl transition duration-300" aria-label="Learn more about this service">
                      Learn More
                    </button>
                  </Link>
                </div>
      ...
      <section className="py-20 bg-gray-800 text-gray-200">
        <div className="container mx-auto text-center">
          ...
          <Link to="/booking">
            <button className="mt-8 px-10 py-4 bg-gray-900 text-white border border-gray-600 rounded-full shadow-lg hover:bg-gray-700 hover:shadow-xl transition-all ease-in-out duration-300" aria-label="Book your trip now">
              Book Your Trip
            </button>
          </Link>
        </div>
